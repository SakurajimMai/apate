...
`.github/workflows/release.yml` 会在 `main` push 和 `v*` tag push 时构建 Windows/Linux 产物。`main` push 会把附件上传到 `latest` 预发布 Release；`v*` tag 会创建正式 GitHub Release，并使用 `CHANGELOG.md` 的 `Unreleased` 段作为 Release Notes。
