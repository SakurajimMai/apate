...
- 只处理用户明确给出的目标路径，不要因为当前工作区存在文件就自动扩大范围。
- 不要把 `local`、`test`、`tests`、`fixtures`、`samples`、`target`、`.git` 等本地目录或测试样例文件写入、伪装、还原或打包进交付附件。
- 如果用户给的是项目根目录，先列出将处理的真实文件并 dry-run；测试文件和本地临时目录默认排除，除非用户点名要求处理。
- 发布附件只应包含目标平台应用产物，例如 `apate.exe`、Linux 二进制或 Android APK，不要夹带本地测试文件、临时目录或仓库测试资源。
